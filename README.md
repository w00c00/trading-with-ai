...
- Dashboard：http://127.0.0.1:3471/dashboard
- 策略交易：http://127.0.0.1:3471/trade
- 批量/模拟：http://127.0.0.1:3471/batch
...
- 策略模拟：不调用 AI、不下单，只用历史 K 线评估策略，并绘制价格和权益曲线
...
上传成功后策略会立即出现在策略开关里，并保存到 `.config/strategies/*.json`。
上传后的自定义策略也可以在“批量/模拟”里勾选，并用指定时间段做模拟测试。
